...
export const registerPlayerEvents = (Nica: NicaMusic, player: Player): void => {
    try {
        player.events.on('connection', (queue) => {
            ...
            logger.debug(
                `Connection in ${queue.guild.name}`,
                'Debug from player event: connection',
            );
        });
        ...
        player.events.on('playerStart', async (queue, track) => {
            ...
            if (!Nica.playerConfigs.loopMessage && queue.repeatMode !== 0) {
                return;
            }
        ...
        player.events.on('audioTracksAdd', (queue) => {
            ...
            if (!metadata.spotifySearching) {
                ...
                (metadata.channel as any).send(
                    'All the tracks in playlist added into the queue ✅',
                );
            }
        });
